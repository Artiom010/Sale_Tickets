...
function formatDetalii($detalii, $n, $locuri_libere = 5) {
...
// Handler pentru /start cump_N sau /cump_N
...
foreach($matches as $idx => $m){
    ...
    // Dacă vrei să afișezi locuri libere precis, poți extrage aici cu API, vezi exemplul comentat:
    $locuri_libere = '?';
    
    // Opțional (doar dacă vrei precis): 
    ...
    $cmd = "cump_$idx";
    $tg_link = "https://t.me/$bot_username?start=$cmd";
    ...
    $text_msg .= "🔗 <a href=\"$tg_link\">Cumpără bilet pentru această cursă</a>\n";
    $text_msg .= "────────────────────────\n";
}
...
// 4. Trimite lista bogată cu toate cursele și butoane verticale
...
//     // Folosește editMessageText!
//     editMessageText($chat_id, $state['places_msgid'], formatDetalii($detalii, $n), makePlacesKeyboard($n));
//     exit;
// }
...
if($data === 'final_conf' && $state['step'] == 'choose_places'){
    ...
    $msg = "<b>Rezervare:</b>\n".
           "Direcție: {$detalii['plecare']}\n".
           "Data: {$detalii['data']}\n".
           "Ora: {$detalii['ora']}\n".
           "Ruta: {$detalii['ruta']}\n".
           "Locuri: $nr_loc\n".
           "Preț pe loc: {$detalii['tarif']}\n".
           "<b>Total de plată:</b> $pret_total_fmt\n".
           "Rezervare completă!";
...
if ($data === 'pay_card') {
    ...
    $body = [
        ...
        "orderId"     => $orderId,
            ...
            // "okUrl" => "https://exemplu.ro/maib_complete.php?chat_id={$chat_id}&payId={$payId}" //for maib complete php
...
//         // ... aici logica de generare bilet/PDF ...
//         clearState($chat_id);
//     } else {
...
if ($data === 'pay_cash') {
    ...
    $msg = "Plată DEMO cash:\n\n" .
        "1️⃣ Mergi la terminal DEMO sau <a href='https://fc0c-89-28-112-18.ngrok-free.app/plata-cash-demo.php?code=$cod_cash'>apasa aici pentru a marca ca platit</a>\n".
    ...
    while ($waited < $max_wait) {
        ...
        if (checkDemoCashPaid($cod_cash)) {
            // --- CONFIRMAT, continui ca la QIWI ---
            sendMessage($chat_id, "✅ Plata cash a fost procesată cu succes! Rezervarea este confirmată.");

            // --- Aici GENERARE BON PDF! ---
            // vezi codul de la generarea PDF-ului la plată Qiwi (ai deja mai sus în cod)
            // exemplu simplificat:
            require_once(__DIR__.'/fpdf/fpdf.php');
            $pdf = new FPDF('P', 'mm', [80, 120]);
            $pdf->AddPage();
...
if(isset($state['pay_cash_demo_code'])){
    ...
    if(file_exists($file)){
        ...
        // Poți pune un timer JS sau un buton "Am plătit"
